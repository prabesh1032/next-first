...
const SERVICES = [
  {
    ...
    includes: ["Multi-cloud provisioning", "Autoscaling & load balancing", "Monthly cost reviews"],
  },
  {
    icon: ShieldCheck,
    title: "Security & compliance",
    body: "Hardening and audit trails wired in from day one, not bolted on before a review.",
    includes: ["SOC 2-ready controls", "Vulnerability scanning", "Access & audit logging"],
  },
  {
    icon: Activity,
    title: "24/7 monitoring",
    body: "Real engineers watching real dashboards, with an SLA that's actually enforced.",
    includes: ["15-minute response SLA", "Custom alert thresholds", "Monthly incident reports"],
  },
  {
    icon: GitBranch,
    title: "DevOps automation",
    body: "CI/CD pipelines and infrastructure-as-code that ship changes without drama.",
    includes: ["Terraform / IaC setup", "CI/CD pipeline builds", "Automated rollback paths"],
  },
  {
    icon: Server,
    ...
    includes: ["Follow-the-sun coverage", "Runbook creation", "Postmortem facilitation"],
  },
  {
    icon: Lock,
    title: "Access & identity management",
    body: "Least-privilege access across every system, reviewed on a schedule, not by accident.",
    includes: ["SSO / IdP integration", "Quarterly access reviews", "Offboarding automation"],
  },
];
...
const TIERS = [
  {
    ...
    features: ["24/7 monitoring & alerting", "Monthly incident report", "Business-hours support"],
  },
  {
    name: "Managed Ops",
    price: "$4,800",
  ...
  return (
    <div className="border-b border-white/8 py-5">
      ...
      >
        ...
        <span
          className={
            "shrink-0 text-[#22D3EE] transition-transform " + (open ? "rotate-45" : "")
          }
          aria-hidden
        >
          +
      ...
      {open && <p className="mt-3 max-w-2xl text-sm leading-6 text-[#9BA5B7]">{a}</p>}
    </div>
  );
}
      ...
      <main className="flex-1">
        ...
        <section className="mx-auto max-w-3xl px-6 py-24 text-center lg:py-32">
          ...
          <p className="mx-auto mt-6 max-w-xl text-[17px] leading-7 text-[#9BA5B7]">
            Every engagement starts with the parts of your stack that keep
            you up at night. Add coverage as you need it.
          </p>
        </section>
            ...
            {SERVICES.map(({ icon: Icon, title, body, includes }) => (
              ...
              >
                ...
                <ul className="mt-4 space-y-2">
                  {includes.map((item) => (
                    <li key={item} className="flex items-start gap-2 text-xs text-[#9BA5B7]">
        ...
        <section className="border-y border-white/5 bg-[#0A0E17]">
          <div className="mx-auto max-w-6xl px-6 py-24">
            <div className="mb-14 max-w-xl">
              <h2 className="text-3xl font-semibold tracking-tight">Coverage tiers.</h2>
              <p className="mt-3 text-[#9BA5B7]">Straightforward pricing, no seat-based math.</p>
            </div>
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              {TIERS.map((tier) => (
                ...
                >
                  {tier.highlighted && (
                    <span className="absolute -top-3 left-8 rounded-full bg-gradient-to-r from-[#22D3EE] to-[#8B5CF6] px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#05070D]">
                      Most common
                    </span>
                  )}
                  ...
                  <p className="mt-6 flex items-baseline gap-1">
                    ...
                    <span className="text-sm text-[#7C8698]">{tier.period}</span>
                  </p>
                  <ul className="mt-6 space-y-3">
                    {tier.features.map((f) => (
                      <li key={f} className="flex items-start gap-2 text-sm text-[#9BA5B7]">
                  ...
                  <Link
                    ...
                    className={
                      ...
                      (tier.highlighted
                        ? "bg-gradient-to-r from-[#22D3EE] to-[#8B5CF6] text-[#05070D]"
        ...
        <section className="mx-auto max-w-3xl px-6 py-24">
          <h2 className="mb-8 text-3xl font-semibold tracking-tight">Common questions.</h2>
          <div>
            {FAQS.map((f) => (
